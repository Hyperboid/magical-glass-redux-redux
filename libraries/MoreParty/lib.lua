...
function Lib:init()
    ...
    Utils.hook(ActionBoxDisplay, "draw", function(orig, self)
        ...
        if not Kristal.getLibConfig("moreparty", "classic_mode") then
            Draw.setColor(MagicalGlassLib and Kristal.getLibConfig("magical-glass", "light_world_dark_battle_color_override") and Game:isLight() and (Game.battle.encounter.karma_mode and MG_PALETTE["player_karma_health_bg"] or MG_PALETTE["player_health_bg"]) or PALETTE["action_health_bg"]) -- Compatibility with  'magical-glass' Library.
            ...
            if health > 0 then
                ...
                if MagicalGlassLib and Kristal.getLibConfig("magical-glass", "light_world_dark_battle_color_override") and Game:isLight() then -- Compatibility with 'magical-glass' Library.
                    Draw.setColor(MG_PALETTE["player_health"])
                else
                    Draw.setColor(self.actbox.battler.chara:getColor())
                end
                love.graphics.rectangle("fill", 118, 22 - self.actbox.data_offset, math.min(math.ceil(MagicalGlassLib and (Game.battle.encounter.karma_mode and karma_health > 1 and (karma_health - 1) or karma_health) or health), 39), 9)
            end
